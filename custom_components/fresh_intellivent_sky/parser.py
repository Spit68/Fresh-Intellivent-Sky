...
class SkyModeParser:
    ...
    def boost_read(self, value: Union[bytes, bytearray]) -> dict[str, Union[bool, int]]:
        """Parse the current Boost state from the device."""
        if len(value) != 5:
            raise ValueError(f"Length need to be exactly 5, was {len(value)}.")

        active_raw, fan_speed, time_remaining = unpack("<BHH", value)
    ...
    def boost_write(self, seconds: int, fan_speed: int) -> bytes:
        """Start Boost for the requested number of seconds, or cancel with zero."""
        seconds = int(seconds)
        fan_speed = int(fan_speed)

        if not 0 <= seconds <= 3600:
            raise ValueError(
                f"Boost duration must be between 0 and 3600 seconds, was {seconds}."
            )
        ...
        payload = pack("<?HH", active, fan_speed, seconds)
    ...
    def voc_read(self, value: Union[bytes, bytearray]) -> dict[str, int]:
        """Parse VOC mode RPM from special settings page 5."""
        if len(value) != 5:
            raise ValueError(f"Length need to be exactly 5, was {len(value)}.")

        page, rpm, humidity_dec_stop, reserved = unpack(">BHBB", value)

        if page != 0x05:
            raise ValueError(
                f"Expected special settings page 5 (0x05), got 0x{page:02X}."
            )

        return {
            "rpm": int(rpm),
            "humidity_dec_stop": int(humidity_dec_stop),
    ...
    def voc_write(
        ...
        """Write VOC mode RPM to special settings page 5."""
        return pack(
            ">BHBB",
            0x85,
            h.validated_rpm(rpm),
            int(humidity_dec_stop),
            int(reserved),
        )
